...
class ManagerReportController extends Controller
{
    ...
    public function export(Request $request): StreamedResponse
    {
        ...
        $filename = 'manager-report-' . now()->format('Ymd-His') . '.csv';

        return response()->streamDownload(function () use ($rows): void {
            $handle = fopen('php://output', 'w');
            fputcsv($handle, ['Branch', 'City', 'Outgoing Shipments', 'Incoming Shipments', 'Outgoing Revenue Delivered']);

            foreach ($rows as $row) {
                fputcsv($handle, [
                    $row->name,
                    $row->city,
                    $row->outgoing_shipments,
                    $row->incoming_shipments,
                    (float) ($row->outgoing_revenue ?? 0),
                ]);
            }
